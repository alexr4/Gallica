class JSONRecorder extends Thread {
  ...
  JSONRecorder(PApplet parent, String name, int index, int maxDocument, String request, String type) {
    super(name);
    ...
    saveJSONObject(this.savedDataFile, "data/gallica_"+this.type+documentIteration+".json");
    ...
    if (cursor < numberOfDocuments) {
      try {
        ...
        if (!resumptionToken.equals("")) { 
          ...
          saveJSONObject(savedDataFile, "data/gallica_"+type+documentIteration+".json");
          mt.addSample();
        } else {
